...
- 원인 분석 : 파이프라인이 AWS에 접속할 권한은 정상적으로 획득했으나 Docker가 ECR에 로그인하는 절차가 누락된 것을 발견
- 시도한 방법 : GitHub Actions 파이프라인의 단계별 에러 로그 분석
- 최종 해결 : 파이프라인에 aws-actions/amazon-ecr-login@v2 단계를 추가하여 ECR 자동 로그인 구현
- 배운 점 : AWS 인프라 접근 권한을 얻는 것과 Docker가 ECR에 로그인하는 과정이 독립된 절차임을 이해
